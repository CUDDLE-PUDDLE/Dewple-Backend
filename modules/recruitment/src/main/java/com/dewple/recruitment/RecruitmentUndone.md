...
> **결정:**
> - 공고 내용(제목/설명/컴포넌트) + 지원서 양식 수정: 게시(OPEN) ~ 마감 2일 전까지
> - 모임 기간/인원 변경: 기존 규칙 유지, 2일 전 제한 미적용
> - 마감일 변경: 횟수 제한(2회)만 유지, 2일 전 제한 미적용
> - 추가합격 기간 중: 마감 이후이므로 수정 불가
>
> **반영 내용:** `updateRecruitment()`에 `validateContentEditDeadline()` 검증 추가 (endAt - 2일 기준). 에러코드 `POSTING_EDIT_DEADLINE_PASSED`(5118) 추가. `changeDeadline()`은 기존 로직 유지.
