...
class ApplicationControllerTest {
    ...
    @Nested
    @DisplayName("POST /clubs/{clubId}/recruitment-posts/{postingId}/applications/guest — 비회원 지원서 제출")
    class SubmitGuestApplicationTest {

        @Test
        @DisplayName("성공: 비회원 지원서 제출")
        void success() throws Exception {
            // given
            Application application = createMockApplication(500L, ApplicationStatus.SUBMITTED);
            given(applicationService.submitGuestApplication(eq(1L), eq(100L), any()))
                    .willReturn(application);

            // when & then
            mockMvc.perform(post("/clubs/{clubId}/recruitment-posts/{postingId}/applications/guest",
                            1L, 100L)
                            .contentType(MediaType.APPLICATION_JSON)
        ...
        @Test
        @DisplayName("실패: 서비스에서 GUEST_APPLICATION_ALREADY_SUBMITTED → 409, code=5108")
        void failDuplicate() throws Exception {
            // given
            willThrow(new BusinessException(RecruitmentErrorCode.GUEST_APPLICATION_ALREADY_SUBMITTED))
                    .given(applicationService).submitGuestApplication(eq(1L), eq(100L), any());
        ...
        @Test
        @DisplayName("실패: 유효성 검증 실패 (전화번호 누락)")
        void failValidation() throws Exception {
            Map<String, Object> invalidRequest = Map.of(
                    "answers", List.of(Map.of("key", "q1", "value", "답변"))
            );
    ...
    @Nested
    @DisplayName("PATCH /clubs/{clubId}/recruitment-posts/{postingId}/applications/guest — 비회원 지원서 수정")
    class EditGuestApplicationTest {
        ...
        void success() throws Exception {
            // given
            Application application = createMockApplication(500L, ApplicationStatus.SUBMITTED);
            given(applicationService.editGuestApplication(eq(1L), eq(100L), any()))
                    .willReturn(application);

            // when & then
            mockMvc.perform(patch("/clubs/{clubId}/recruitment-posts/{postingId}/applications/guest",
                            1L, 100L)
                            .contentType(MediaType.APPLICATION_JSON)
        ...
        @Test
        @DisplayName("실패: 서비스에서 GUEST_APPLICATION_NOT_FOUND → 404, code=5109")
        void failNotFound() throws Exception {
            // given
            willThrow(new BusinessException(RecruitmentErrorCode.GUEST_APPLICATION_NOT_FOUND))
                    .given(applicationService).editGuestApplication(eq(1L), eq(100L), any());
    ...
    private Map<String, Object> createGuestRequest() {
        return Map.of(
    ...
    private Map<String, Object> createUpdateGuestRequest() {
        return Map.of(
                "guestPhone", "01012345678",
                "answers", List.of(
                        Map.of("key", "q1", "value", "수정된 답변입니다.")
                )
        );
    }
}
